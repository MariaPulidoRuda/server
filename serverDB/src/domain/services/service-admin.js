...
exports.PostAdmin = async (req, res) => {
  ...
  try {
    ...
    if (AdminName && Password ) {
      let respOrm = await ormAdmin.PostAdmin(
        AdminName,
        Password,
      );
      if (respOrm.err) {
...
exports.DeleteAdmin = async (req, res) => {
  ...
  try {
    ...
    if (id) {
      let respOrm = await ormAdmin.DeleteAdmin(id);
      console.log(respOrm);
      if (respOrm.err) {
        (status = 'Failure'),
          (errorcode = respOrm.err.code),
        ...
        (message = 'Admin deleted'), (statuscode = enum_.CODE_OK);
      }
    } else {
      (status = 'Failure'),
  ...
  try {
    ...
    if (id && updatedAdmin) {
      ...
      if (respOrm.err) {
        ...
        (message = 'Admin updated'), (statuscode = enum_.CODE_OK);
      }
    } else {
      (status = 'Failure'),
        (errorcode = enum_.ERROR_REQUIRED_FIELD), 
...
exports.Admin = async (req, res) => {
  ...
  try {
    ...
    let respOrm = await ormAdmin.Admin(id);
    if (respOrm.err) {
      (status = 'Failure'),
        (errorcode = respOrm.err.code),
        (message = respOrm.err.message),
      ...
      (message = 'Success getting the admin'),
        (data = respOrm),
...
//get by name
exports.AdminName = async (req, res) => {
  let status = 'Success';
  let errorcode = '';
  let message = '';
  let data = '';
  ...
  try {
    ...
    let respOrm = await ormAdmin.AdminName(name);
...
exports.Login = async (req, res) => {
  ...
  try {
    ...
    if (AdminName && Password) {
      let respOrm = await ormAdmin.Login(AdminName, req);
